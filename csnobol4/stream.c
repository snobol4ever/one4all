...
#include <string.h>
int g_csno_trace = 0;
int g_csno_trace_fd = 2;  /* default stderr fd */
#endif
...
stream(
    ...
    /* capture for TRACE_STREAM */
    const char *orig_ptr = S_SP(sp2);
    int          orig_len = S_L(sp2);
    const char  *tp_name  = tp->name;
 ...
 break_loop:
    ...
    /* TRACE_STREAM: emit identical format to sno4parse's g_trace_stream output */
#ifdef TRACE_STREAM
    if (g_csno_trace) {
        char ibuf[32];
        int ilen = orig_len < 20 ? orig_len : 20;
        char line[128]; int n;
        memcpy(ibuf, orig_ptr, ilen); ibuf[ilen] = '\0';
        for (int i = 0; i < ilen; i++)
            if ((unsigned char)ibuf[i] < 32 || ibuf[i] == 127) ibuf[i] = '.';
        const char *retname = ret==ST_STOP?"STOP":ret==ST_EOS?"EOS":"ERROR";
        n = snprintf(line, sizeof(line), "STREAM %-10s [%-20s] -> ret=%-5s stype=%d\n",
                tp_name, ibuf, retname, (int)put);
        write(g_csno_trace_fd, line, n);
    }
#endif
    return ret;
}
