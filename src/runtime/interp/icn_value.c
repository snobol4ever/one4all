/*============================================================================================================================
 * icn_value.c — RS-17a: pure-BB value-context evaluator for Icon Byrd boxes.
 *
 * See icn_value.h for the contract and migration plan.
 *
 ...
 * for the incremental migration of icn_runtime.c call sites (RS-17a-cont).
 *
 ...
 * through `icn_descr_identical` (declared in icn_runtime.h).  Note: there
 ...
 * via icn_bb_build + bb_node_t.fn ticks), and plain `lv OP rv` then
 ...
 * the `interp_eval` extern be dropped (RS-23) and icn_value.c
 ...
 * to interp_eval today; routing them through icn_bb_build + a single bb_broker
...
DESCR_t bb_eval_value(tree_t *e)
{
     ...
     *   (2) RHS suspendable          — drive via icn_bb_build+bb_node_t,
     ...
     *
     * (3) Fresh evaluation in value context: build a bb_node_t via icn_bb_build
    ...
    case TT_TO_BY: {
        ...
        /* (3) fresh first-value via icn_bb_build+α */
     ...
     * (now declared in icn_runtime.h).  TT_CSET literal mirrors interp_eval.c:3466. */
    case TT_CSET:
     ...
     *   3. Generic — drive gen via icn_bb_build box, run body per tick. */
    case TT_EVERY: {
        if (e->n < 1) return NULVCL;
        tree_t *gen  = e->c[0];
     ...
     * remaining 5-of-6 unique tuples (`TT_RETURN via icn_bb_build` excluded —
     ...
     *   TT_REVASSIGN    caller=bb_exec_stmt      via=bb_exec_stmt   (in icn_stmt.c)
     ...
     * mirrors the stmt-context handler at icn_stmt.c:106 — for a
     * suspendable cond, drive its first value via icn_bb_build+α so generator
     ...
     * `expr | fail` alternation lives in icn_bb_build (RS-23b's icn_lazy_box
     * wrapping at icn_runtime.c:1576) and is unaffected — the lazy box
     ...
     * reached only when icn_bb_build is asked for a box (every / alt-driven
     ...
     * icn_bb_build.  Outer-pump retries reach it through the box's β path,
     * not through re-entry of bb_eval_value.  Mirrors the existing
     * TT_LIMIT/TT_ALTERNATE/TT_SEQ_EXPR pattern at icn_value.c:888-901. */
    case TT_BANG_BINARY: {
     ...
     * icn_stmt.c:69 is the more common path. */
    case TT_LOOP_BREAK: {
        FRAME.loop_break = 1;
     ...
     * from).  The stmt-context handler in icn_stmt.c:80 is the common
