...
IR_t * IR_exec_node(IR_t * nd) {
    ...
    case IR_PAT_SPAN: {
        ...
        if (nd->state == 0) {
            int i = 0;
            while (Δ + i < Σlen && strchr(chars, Σ[Δ + i])) i++;
            if (i == 0) { nd->value = FAILDESCR; return nd->port_fail; }
        ...
        if (nd->state == 1) {
            ...
            nd->counter--;
            if (nd->counter < 1) { nd->state = 2; nd->value = FAILDESCR; return nd->port_fail; }
     ...
     * The inner pattern is wired as port_start; we fire NV_SET_fn when succ arrives.
    ...
    case IR_PAT_ASSIGN_COND: {
        if (nd->state == 0) {
            ...
            nd->state   = 1;
            nd->value   = NULVCL;
            return nd->port_succ;   /* pass through to inner; wiring routes succ here after inner */
        }
