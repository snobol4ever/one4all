...
static void emit_sm_lbl_rt    (emitter_t *e, const char *comment, const char *macro, const char *rt_sym, const char *lbl, uint64_t ptr);
static void emit_sm_pat_lbl_rt(emitter_t *e, const char *comment, const char *macro, const char *rt_sym, const char *lbl, uint64_t ptr);
...
void emit_sm_mul(emitter_t *e) { emit_sm_arith_op(e, SM_MUL, "MUL_NUM"); }
void emit_sm_div(emitter_t *e) { emit_sm_arith_op(e, SM_DIV, "DIV_NUM"); }
void emit_sm_mod(emitter_t *e) { emit_sm_arith_op(e, SM_MOD, "MOD_NUM"); }
/*====================================================================================================================*/
...
void emit_sm_pat_refname  (emitter_t *e, const char *lbl, uint64_t ptr) { emit_sm_pat_lbl_rt(e, "SM_PAT_REFNAME — push *varname pattern (deref by name)", "PAT_REFNAME",  "rt_pat_refname",  lbl, ptr); }
void emit_sm_pat_usercall (emitter_t *e, const char *lbl, uint64_t ptr) { emit_sm_pat_lbl_rt(e, "SM_PAT_USERCALL — push *func() user-call pattern",       "PAT_USERCALL", "rt_pat_usercall", lbl, ptr); }
/*====================================================================================================================*/
void emit_sm_pat_eps    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_EPS — push epsilon pattern",                    "PAT_EPS",     "rt_pat_eps");     }
void emit_sm_pat_arb    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_ARB — push ARB (greedy 0+) pattern",            "PAT_ARB",     "rt_pat_arb");     }
void emit_sm_pat_rem    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_REM — push REM (rest of subject) pattern",      "PAT_REM",     "rt_pat_rem");     }
void emit_sm_pat_fail   (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_FAIL — push FAIL (always fail) pattern",        "PAT_FAIL",    "rt_pat_fail");    }
void emit_sm_pat_succeed(emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_SUCCEED — push SUCCEED (always succeed) pat",   "PAT_SUCCEED", "rt_pat_succeed"); }
void emit_sm_pat_abort  (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_ABORT — push ABORT (terminate match) pattern",  "PAT_ABORT",   "rt_pat_abort");   }
void emit_sm_pat_bal    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_BAL — push BAL (balanced string) pattern",      "PAT_BAL",     "rt_pat_bal");     }
void emit_sm_pat_fence  (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_FENCE0 — push FENCE (no-backtrack gate)",       "PAT_FENCE",   "rt_pat_fence");   }
void emit_sm_pat_fence1 (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_FENCE1 — pop child, push FENCE(child)",         "PAT_FENCE1",  "rt_pat_fence1");  }
void emit_sm_pat_span   (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_SPAN — pop charset, push SPAN(cs)",             "PAT_SPAN",    "rt_pat_span");    }
void emit_sm_pat_break  (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_BREAK — pop charset, push BREAK(cs)",           "PAT_BREAK",   "rt_pat_break");   }
void emit_sm_pat_any    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_ANY — pop charset, push ANY(cs)",               "PAT_ANY",     "rt_pat_any");     }
void emit_sm_pat_notany (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_NOTANY — pop charset, push NOTANY(cs)",         "PAT_NOTANY",  "rt_pat_notany");  }
void emit_sm_pat_len    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_LEN — pop integer n, push LEN(n)",              "PAT_LEN",     "rt_pat_len");     }
void emit_sm_pat_pos    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_POS — pop integer n, push POS(n)",              "PAT_POS",     "rt_pat_pos");     }
void emit_sm_pat_rpos   (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_RPOS — pop integer n, push RPOS(n)",            "PAT_RPOS",    "rt_pat_rpos");    }
void emit_sm_pat_tab    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_TAB — pop integer n, push TAB(n)",              "PAT_TAB",     "rt_pat_tab");     }
void emit_sm_pat_rtab   (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_RTAB — pop integer n, push RTAB(n)",            "PAT_RTAB",    "rt_pat_rtab");    }
void emit_sm_pat_arbno  (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_ARBNO — pop child pattern, push ARBNO(child)",  "PAT_ARBNO",   "rt_pat_arbno");   }
void emit_sm_pat_cat    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_CAT — pop right+left patterns, push CAT(l,r)",  "PAT_CAT",     "rt_pat_cat");     }
void emit_sm_pat_alt    (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_ALT — pop right+left patterns, push ALT(l,r)",  "PAT_ALT",     "rt_pat_alt");     }
void emit_sm_pat_deref  (emitter_t *e) { emit_sm_rtcall(e, "SM_PAT_DEREF — pop value, deref to pattern",           "PAT_DEREF",   "rt_pat_deref");   }
/*====================================================================================================================*/
void emit_sm_push_var  (emitter_t *e, const char *lbl, uint64_t ptr) { emit_sm_lbl_rt(e, "SM_PUSH_VAR — push value of named variable via rt_nv_get",   "PUSH_VAR",  "rt_nv_get", lbl, ptr); }
void emit_sm_store_var (emitter_t *e, const char *lbl, uint64_t ptr) { emit_sm_lbl_rt(e, "SM_STORE_VAR — store TOS into named variable via rt_nv_set", "STORE_VAR", "rt_nv_set", lbl, ptr); }
/*====================================================================================================================*/
void emit_sm_concat         (emitter_t *e) { emit_sm_rtcall(e, "SM_CONCAT — pop right+left, push concat result",         "CONCAT",         "rt_concat");          }
void emit_sm_push_null      (emitter_t *e) { emit_sm_rtcall(e, "SM_PUSH_NULL — push null descriptor",                    "PUSH_NULL",      "rt_push_null");       }
void emit_sm_push_null_noflip(emitter_t *e){ emit_sm_rtcall(e, "SM_PUSH_NULL_NOFLIP — push null, preserve last_ok",      "PUSH_NULL_NOFLIP","rt_push_null_noflip");}
void emit_sm_coerce_num     (emitter_t *e) { emit_sm_rtcall(e, "SM_COERCE_NUM — coerce TOS string to number",            "COERCE_NUM",     "rt_coerce_num");      }
void emit_sm_exp            (emitter_t *e) { emit_sm_rtcall(e, "SM_EXP — pop 2, push base**exp",                         "EXP_NUM",        "rt_exp");             }
void emit_sm_neg            (emitter_t *e) { emit_sm_rtcall(e, "SM_NEG — negate TOS",                                    "NEGATE",         "rt_neg");             }
void emit_sm_define         (emitter_t *e) { emit_sm_rtcall(e, "SM_DEFINE — no-op: function definition prescan",         "DEFINE",         "rt_define");          }
void emit_sm_define_entry   (emitter_t *e) { emit_sm_rtcall(e, "SM_DEFINE_ENTRY — no-op: function entry marker",         "DEFINE_ENTRY",   "rt_define_entry");    }
/*====================================================================================================================*/
void emit_sm_resume         (emitter_t *e) { emit_sm_rtcall(e, "SM_RESUME — generator resume marker (M5)",               "RESUME",         "rt_unhandled_sm"); }
void emit_sm_suspend        (emitter_t *e) { emit_sm_rtcall(e, "SM_SUSPEND — generator suspend (M5)",                    "SUSPEND",        "rt_unhandled_sm"); }
void emit_sm_suspend_value  (emitter_t *e) { emit_sm_rtcall(e, "SM_SUSPEND_VALUE — coroutine yield (M5)",                "SUSPEND_VALUE",  "rt_unhandled_sm"); }
void emit_sm_gen_tick       (emitter_t *e) { emit_sm_rtcall(e, "SM_GEN_TICK — drive generator one tick (M5)",            "GEN_TICK",       "rt_unhandled_sm"); }
void emit_sm_load_glocal    (emitter_t *e) { emit_sm_rtcall(e, "SM_LOAD_GLOCAL — push gen-local slot (M5)",              "LOAD_GLOCAL",    "rt_unhandled_sm"); }
void emit_sm_store_glocal   (emitter_t *e) { emit_sm_rtcall(e, "SM_STORE_GLOCAL — pop into gen-local slot (M5)",         "STORE_GLOCAL",   "rt_unhandled_sm"); }
void emit_sm_load_frame     (emitter_t *e) { emit_sm_rtcall(e, "SM_LOAD_FRAME — push IcnFrame slot (M5)",                "LOAD_FRAME",     "rt_unhandled_sm"); }
void emit_sm_store_frame    (emitter_t *e) { emit_sm_rtcall(e, "SM_STORE_FRAME — pop into IcnFrame slot (M5)",           "STORE_FRAME",    "rt_unhandled_sm"); }
void emit_sm_icmp_gt        (emitter_t *e) { emit_sm_rtcall(e, "SM_ICMP_GT — integer compare > (M5)",                   "ICMP_GT",        "rt_unhandled_sm"); }
void emit_sm_icmp_lt        (emitter_t *e) { emit_sm_rtcall(e, "SM_ICMP_LT — integer compare < (M5)",                   "ICMP_LT",        "rt_unhandled_sm"); }
void emit_sm_bb_once        (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_ONCE — run BB once (M5)",                          "BB_ONCE",        "rt_unhandled_sm"); }
void emit_sm_bb_once_proc   (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_ONCE_PROC — Prolog proc BB once (M5)",             "BB_ONCE_PROC",   "rt_unhandled_sm"); }
void emit_sm_bb_pump        (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP — drive BB generator (M5)",                   "BB_PUMP",        "rt_unhandled_sm"); }
void emit_sm_bb_pump_case   (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP_CASE — Raku case BB pump (M5)",               "BB_PUMP_CASE",   "rt_unhandled_sm"); }
void emit_sm_bb_pump_every  (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP_EVERY — every-generator BB pump (M5)",        "BB_PUMP_EVERY",  "rt_unhandled_sm"); }
void emit_sm_bb_pump_proc   (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP_PROC — Icon proc BB pump (M5)",               "BB_PUMP_PROC",   "rt_unhandled_sm"); }
void emit_sm_bb_pump_sm     (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP_SM — migrated SM BB pump (M5)",               "BB_PUMP_SM",     "rt_unhandled_sm"); }
void emit_sm_bb_pump_ast    (emitter_t *e) { emit_sm_rtcall(e, "SM_BB_PUMP_AST — drive BB generator via AST pump",       "BB_PUMP_AST",    "rt_bb_pump_ast"); }
/*====================================================================================================================*/
