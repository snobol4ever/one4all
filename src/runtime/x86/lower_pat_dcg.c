/*
 * lower_pat_dcg.c -- build ir_graph_t (DCG) from SNOBOL4 pattern tree_t (LR-S1)
 ...
 * wiring pass from tree_t*. On success, caller stores ir_graph_t* in SM_EXEC_STMT a[2].ptr.
...
static ir_node_t * build_node(ir_graph_t * cfg, const tree_t * t, ir_node_t * sp, ir_node_t * fp);
static ir_node_t * build_node(ir_graph_t * cfg, const tree_t * t, ir_node_t * sp, ir_node_t * fp) {
    if (!t) return sp;
    ir_node_t * nd = NULL;
    switch (t->t) {
    case TT_QLIT: {
        nd = ir_alloc_node(cfg, IR_PAT_LIT, IR_LANG_SNO);
        nd->sval = t->v.sval ? t->v.sval : "";
        nd->port_start = nd; nd->port_resume = fp; nd->port_succ = sp; nd->port_fail = fp;
        return nd;
    }
    case TT_ARB: {
        nd = ir_alloc_node(cfg, IR_PAT_ARB, IR_LANG_SNO);
        nd->generative = 1;
        nd->port_start = nd; nd->port_resume = nd; nd->port_succ = sp; nd->port_fail = fp;
        return nd;
    }
    case TT_REM: {
        nd = ir_alloc_node(cfg, IR_PAT_REM, IR_LANG_SNO);
        nd->port_start = nd; nd->port_resume = fp; nd->port_succ = sp; nd->port_fail = fp;
        return nd;
    }
    case TT_ABORT: {
        nd = ir_alloc_node(cfg, IR_PAT_ABORT, IR_LANG_SNO);
        nd->port_start = nd; nd->port_resume = fp; nd->port_succ = fp; nd->port_fail = fp;
        return nd;
    }
    case TT_SPAN: {
        if (t->n < 1 || !t->c[0] || t->c[0]->t != TT_QLIT) return NULL;
        nd = ir_alloc_node(cfg, IR_PAT_SPAN, IR_LANG_SNO);
    ...
    case TT_ANY: {
        if (t->n < 1 || !t->c[0] || t->c[0]->t != TT_QLIT) return NULL;
        nd = ir_alloc_node(cfg, IR_PAT_ANY, IR_LANG_SNO);
        nd->sval = t->c[0]->v.sval ? t->c[0]->v.sval : "";
        nd->port_start = nd; nd->port_resume = fp; nd->port_succ = sp; nd->port_fail = fp;
        return nd;
    }
    case TT_BREAK: {
        if (t->n < 1 || !t->c[0] || t->c[0]->t != TT_QLIT) return NULL;
        nd = ir_alloc_node(cfg, IR_PAT_BREAK, IR_LANG_SNO);
        nd->sval = t->c[0]->v.sval ? t->c[0]->v.sval : "";
        nd->port_start = nd; nd->port_resume = fp; nd->port_succ = sp; nd->port_fail = fp;
        return nd;
    }
    case TT_FENCE: {
        ...
        nd = ir_alloc_node(cfg, IR_PAT_FENCE, IR_LANG_SNO);
    ...
    case TT_CAT: {
        ...
        nd = ir_alloc_node(cfg, IR_PAT_CAT, IR_LANG_SNO);
        nd->generative = 1;
    ...
    case TT_ALT: {
        ...
        nd = ir_alloc_node(cfg, IR_PAT_ALT, IR_LANG_SNO);
        nd->generative = 1;
    ...
    case TT_CAPT_COND_ASGN: {
        ...
        nd = ir_alloc_node(cfg, IR_PAT_ASSIGN_COND, IR_LANG_SNO);
    ...
    case TT_CAPT_IMMED_ASGN: {
        ...
        ir_node_t * inner = build_node(cfg, t->c[0], sp, fp);
        if (!inner) return NULL;
        nd = ir_alloc_node(cfg, IR_PAT_ASSIGN_IMM, IR_LANG_SNO);
        nd->sval = (t->n > 1 && t->c[1] && t->c[1]->v.sval) ? t->c[1]->v.sval : NULL;
        nd->port_start = inner; nd->port_resume = inner->port_resume;
        nd->port_succ = sp; nd->port_fail = fp; nd->generative = 1;
        return nd;
...
ir_graph_t * lower_pat_build_dcg(const tree_t * pat_tree) {
    ...
    ir_graph_t * cfg = ir_graph_alloc(cap, IR_LANG_SNO);
    if (!cfg) return NULL;
    ir_node_t * entry = build_node(cfg, pat_tree, NULL, NULL);
    if (!entry) { ir_graph_free(cfg); return NULL; }
    cfg->entry = entry;
    return cfg;
}
