...
/* x86_opcodes.h — named constants for x86-64 opcode bytes used in emit_core.c.
 * Every raw hex constant in an insn_* binary branch maps to one name here.
 * Multi-byte sequences: the lead byte has its own name; extension bytes are
 * named as <LEAD>_<EXT> or given a descriptive suffix.
 * ModRM/SIB bytes that encode a fixed register combination are named explicitly.
 */
...
/* ---- MOV reg/mem (opcode 89/8B) + common ModRM bytes ----------------- */
...
/* ---- INC r/m with REX_B prefix → r12d/r13d --------------------------- */
...
/* Two-byte (0F xx) branch opcodes — use ESC prefix + these: */
