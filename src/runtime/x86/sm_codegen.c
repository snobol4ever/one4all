...
#include "emitter_bb_gen.h"              /* EM-MODE4-IS-MODE3-DUMP-c: capture-and-flush adapter */
 ...
 * handler in sm_interp.c. The synthesised TT_FNC + emit_push_expr path is
 ...
 * Body-fn is NULL — the TT_EVERY do-clause (the user's body, e.g.
...
static void h_store_var(void)
{
     ...
     * NV_SET_fn.  The IR path (interp.c TT_ASSIGN, line 2844) always returns
...
static void init_handler_table(void)
{
     ...
     * TT_FNC + SM_PUSH_EXPR + SM_BB_PUMP wrapper for top-level call_main. */
...
int sm_codegen(SM_Program *prog)
{
    ...
    for (int i = 0; i < prog->count; i++) {
        ...
        /* ESP-11: all opcodes use emit_standard_blob(g_handlers[op]) via the
         * unified handler table.  Four structural cases are preserved:
         *   SM_HALT      — emit_halt_blob (ret instead of jmp trampoline)
         *   SM_JUMP      — pass-2 rel32 patch record required
         *   SM_JUMP_S/F  — two pass-2 rel32 patch records required
         *   SM_LABEL     — stackless no-op blob (no r12 sync needed)
         *   SM_STNO      — no-stack blob (sp resets to 0; r12 reloaded after)
         * All other opcodes route through emit_standard_blob, which syncs
         * r12<->st->sp around the C handler call. */
        switch (op) {
        case SM_HALT:
            emit_halt_blob();
            break;
