...
typedef enum {
    ...
    /* LR-3: SM_EXEC_DCG — drive a compile-time wired ir_graph_t* once.
    ...
    SM_EXEC_DCG,
    /* LR-3: SM_PUMP_DCG — drive a compile-time wired ir_graph_t* to exhaustion.
     * For generative contexts (every, while, gather): replaces SM_BB_PUMP+table-id.
     * a[0].ptr = ir_graph_t* (same DCG pointer as SM_EXEC_DCG).
