...
static void lower_qlit(const AST_t *t) { sm_emit_s(g_p, SM_PUSH_LIT_S, t->v.sval ? t->v.sval : ""); }
static void lower_cset(const AST_t *t) { sm_emit_s(g_p, SM_PUSH_LIT_S, t->v.sval ? t->v.sval : ""); }
static void lower_ilit(const AST_t *t) { sm_emit_i(g_p, SM_PUSH_LIT_I, (int64_t)t->v.ival); }
static void lower_flit(const AST_t *t) { sm_emit_f(g_p, SM_PUSH_LIT_F, t->v.dval); }
static void lower_nul (const AST_t *t) { (void)t; sm_emit(g_p, SM_PUSH_NULL); }

/*── Variable references ─────────────────────────────────────────────────────*/

static void lower_var(const AST_t *t)     { emit_var_load(t->v.sval ? t->v.sval : ""); }
static void lower_keyword(const AST_t *t) { sm_emit_s(g_p, SM_PUSH_VAR, kw_canonicalize(t->v.sval)); }
...
static void lower_case(const AST_t *t)
{
    ...
    if (ntriples > 0) {
        AST_t *last_cmp = t->c[1 + (ntriples-1)*3];
