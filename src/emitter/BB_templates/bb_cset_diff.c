/* bb_cset_diff.c — BB template for BB_CSET_DIFF.
   ...
   directly to native code for BB_CSET_DIFF. */
#include "bb_template_common.h"
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void bb_cset_diff(void)    { if (IS_X86) return; if (IS_JVM) return; if (IS_JS) return; if (IS_NET) return; if (IS_WASM) return; }
