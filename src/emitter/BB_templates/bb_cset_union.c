/* bb_cset_union.c — BB template for BB_CSET_UNION.
   ...
   directly to native code for BB_CSET_UNION. */
#include "bb_template_common.h"
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
void bb_cset_union(void)    { if (IS_X86) return; if (IS_JVM) return; if (IS_JS) return; if (IS_NET) return; if (IS_WASM) return; }
