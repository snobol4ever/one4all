...
int sm_jump(void) {
    ...
    if (IS_JVM) {
        ...
        if (target >= 0 && target < g_emit.n && g_emit.in_my_method && g_emit.in_my_method[target])
            emit_textf("    goto_w sm_pc_%d\n", target);
        else if (target >= 0 && target < g_emit.n) {
            emit_textf("    invokestatic rt/SnoRt/halt_tos()V\n");
            emit_textf("    iconst_0\n    invokestatic java/lang/System/exit(I)V\n");
            emit_textf("    return\n");
        } else emit_textf("    goto_w %s\n", end_lbl);
        return 0;
    }
    if (IS_JS)   { emit_textf("_pc = %lld; continue; ", (long long)instr->a[0].i); return 1; }
    if (IS_NET)  { emit_textf("    ldc.i4     %lld\n    stloc      _pc\n    br         NET_DISPATCH\n", (long long)instr->a[0].i); return 1; }
    if (IS_WASM) { emit_textf("          (i32.const %lld) (local.set $pc) (br $lp)\n", (long long)instr->a[0].i); return 1; }
    return 0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sm_jump_s(void) {
    ...
    if (IS_JVM) {
        if (target >= 0 && target < g_emit.n && g_emit.in_my_method && g_emit.in_my_method[target]) {
            emit_textf("    invokestatic rt/SnoRt/last_ok()Z\n");
            emit_textf("    ifeq sm_pc_%d_skip\n", i);
            emit_textf("    goto_w sm_pc_%d\n", target);
            emit_textf("sm_pc_%d_skip:\n", i);
        } else if (target >= 0 && target < g_emit.n) {
            emit_textf("    invokestatic rt/SnoRt/last_ok()Z\n");
            emit_textf("    ifeq sm_pc_%d_skip\n", i);
            emit_textf("    invokestatic rt/SnoRt/halt_tos()V\n");
    ...
    if (IS_JS)   { emit_textf("if (rt.last_ok()) _pc = %lld; else _pc = %d; continue; ", (long long)instr->a[0].i, i + 1); return 1; }
    if (IS_NET)  { emit_textf("    call       bool SnoRt::last_ok()\n    brfalse    NET_L%d\n    ldc.i4     %lld\n    stloc      _pc\n    br         NET_DISPATCH\n", i + 1, (long long)instr->a[0].i); return 1; }
    if (IS_WASM) { emit_textf("          (if (call $sno_last_ok)\n            (then (i32.const %lld) (local.set $pc))\n            (else (i32.const %d)   (local.set $pc)))\n          (br $lp)\n", (long long)instr->a[0].i, i + 1); return 1; }
    return 0;
}
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sm_jump_f(void) {
