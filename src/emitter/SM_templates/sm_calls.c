...
#include "emit_sm.h"
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* Shared body for SM_CALL_FN and SM_SUSPEND_VALUE templates.  EC-UNI-16 justified
 * extraction: across all five backend arms the two opcodes differ in exactly one
 * line (JS arm of suspend emits `rt.set_last_ok(!rt._is_fail(rt._peek()));`
 * after the call_or_jump call).  Carries a real conditional (suspend vs plain
 * call) — meets the EC-UNI-16 rule "justified iff carries a real conditional
 * OR de-duplicates non-trivial computation in >= 2 templates."  Saves ~85 lines
 * of byte-near-duplicate code. */
static int sm_call_or_suspend(int suspend) {
    ...
    if (IS_JS) {
        ...
        if (suspend) emit_textf("rt.set_last_ok(!rt._is_fail(rt._peek())); ");
        return 0;
    }
    if (IS_NET) {
        const char * cname = instr->a[0].s ? instr->a[0].s : "";
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
int sm_call_fn(void)        { return sm_call_or_suspend(0); }
int sm_suspend_value(void)  { return sm_call_or_suspend(1); }
