...
#include "snobol4.h"
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void emit_pat_nary(const tree_t *t, SM_op_t op);
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static int emit_goto(SM_op_t op, const char *target)
{
    if (!target) return -1;
    static const struct {
        const char *name;
        SM_op_t plain, succ, fail;
    } ret_kinds[] = {
    ...
    for (unsigned i = 0; i < sizeof ret_kinds / sizeof ret_kinds[0]; i++) {
        if (strcmp(target, ret_kinds[i].name) == 0) {
            SM_op_t emit_op = (op == SM_JUMP_S) ? ret_kinds[i].succ
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
static void lower_comp(const tree_t *t, SM_op_t op)
{
    ICN_BB_EVAL(t);
    lower_expr(T0(t)); lower_expr(T1(t));
...
static void lower_expr_inner(const tree_t *t)
{
    ...
    case TT_MAP: {
        int nc = t->n;
        for (int i = 0; i < nc; i++) if (t->c[i]) lower_expr(t->c[i]);
        SM_emit_si(g_p, SM_CALL_FN, "raku_map", (int64_t)nc);
        return;
    }
    case TT_GREP: {
        int nc = t->n;
        for (int i = 0; i < nc; i++) if (t->c[i]) lower_expr(t->c[i]);
        SM_emit_si(g_p, SM_CALL_FN, "raku_grep", (int64_t)nc);
        return;
    }
    case TT_SORT: {
        int nc = t->n;
...
static void lower_proc_skeletons(void)
{
    ...
    for (int b = 0; b < PL_PRED_TABLE_SIZE_FWD; b++) {
        for (Pl_PredEntry *e = g_stage2.pl_pred_table.buckets[b]; e; e = e->next) {
            ...
            /* IR-CONSOLIDATE-DCG step 5 (2026-05-20): attach the Prolog BB graph to SM_sequence_t's
             * bb_table and hand the index to pl_bb_register.  Mode-4 standalone path
             * (rt_pl_b_end_register) does the same — it lazy-inits g_stage2.sm so SM_seq_bb_add
             * works without a prior lower() pass. */
            int pl_bb_idx = ir_body ? SM_seq_bb_add(g_p, ir_body) : -1;
            Pl_PredEntry_BB *bb = pl_bb_register(e->key, arity, pl_bb_idx);
            if (bb && e->choice) {
                PlScope sc; sc.n = 0;
                bb->lower_sc = sc;
            }
