 ...
 *
 * ast_gc_clone: deep-copies an tree_t subtree allocating with GC_malloc so
 *   the clone survives after the original calloc'd IR tree is freed.
 *   Used by sm_lower before storing tree_t* into SM_PUSH_EXPR a[0].ptr.
 *
 * code_free: frees a CODE_t (STMT_t list + tree_t trees) allocated via
...
/* Free a CODE_t and all its STMT_t / tree_t nodes (calloc-allocated). */
