...
typedef struct {
    ...
    /* SCT-1c-2: line-buffered emission for >1024-char continuation handling.
     * Forward-declared SNO_LINEBUF; the buffer is sized below the struct. */
    char  linebuf[16384];
    int   linelen;
    /* SCT-1c-3: when the last emitted line was :(RETURN), :(FRETURN), or
     * :(NRETURN), the Gimpel template's tail :(RETURN) can be skipped.
     * Saves ~27% of redundant tail lines (measured on full_snobol4.sno). */
    int   last_was_return;
} sno_ctx_t;
...
static void emit_nl(sno_ctx_t *c) {
    ...
    /* SCT-1c-3: detect a tail :(RETURN/FRETURN/NRETURN) so the DEFINE-tail
     * skip can fire.  Cheap suffix-check on the unfflushed line. */
    {
        int newflag = 0;
        if (len >= 9) {
            /* Check ":(RETURN)" tail */
...
static void emit_stmt(sno_ctx_t *c, const tree_t *s) {
    ...
    if (subj && subj->t == TT_DEFINE && subj->n >= 3) {
        ...
        /* SCT-1c-3: skip the template tail :(RETURN) if the body already
         * ended with a :(RETURN/FRETURN/NRETURN) — they're equivalent and
         * the second is unreachable.  Cleans up ~27% of redundant lines. */
...
int tree_to_sno(const tree_t *ast, FILE *out) {
     ...
     * stays consistent.
     *
     * SCT-1c-2 (2026-05-17): emit `-CASE 0` as the first non-comment
     * directive so name-resolution is case-sensitive in SPITBOL.
     *
     *   - SPITBOL Manual Ch.14 line 9067+: "If N is not 0, then lower-case
     *     names are folded to upper-case (the default).  If N is 0 ...
     *     upper- and lower-case names are separate."
     *   - RULES.md "Case-sensitive name space": SCRIP is case-sensitive
     *     byte-for-byte; emitted SNOBOL4 must match.
     *   - Without -CASE 0, ShiftReduce.sc's `Shift(t,v,s)` (capital S)
     *     and semantic.sc's `shift(p,t)` (lowercase) collapse to a single
     *     SHIFT symbol → SPITBOL ERROR 217 "Duplicate label".
     *
     * SCRIP's SNOBOL4 frontend ignores the -CASE control directive (it
     * is already case-sensitive), so the prelude is harmless there.
     */
