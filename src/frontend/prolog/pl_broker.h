...
/* pl_box_goal_from_ir — build a bb_node_t for any goal IR node (TT_FNC, TT_UNIFY, TT_CUT, ...) */
...
/* pl_box_choice — OR-box over all TT_CLAUSE children of an TT_CHOICE node. */
 ...
 * Used when Pl_PredEntry.entry_pc >= 0 (expression lowered by CH-17d/f). */
bb_node_t pl_box_choice_pc(int entry_pc, Term **caller_args, int arity);

/* pl_box_choice_call — build an OR-box for an TT_FNC user-predicate call goal. */
