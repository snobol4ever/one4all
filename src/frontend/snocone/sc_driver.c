 ...
 *   - propagates errors through sc_lower nerrors
 */
...
Program *sc_compile(const char *source, const char *filename)
{
    ...
    if (!segments) { fprintf(stderr, "sc_compile: out of memory\n"); return NULL; }
    int nseg = 0;
        ...
        if (is_sep) {
            ...
            if (seg_len > 0 && nseg < MAX_SEGMENTS) {
                ...
                if (ccount + pr->count + 1 >= cap) {
                    ...
                    if (!combined) {
                        fprintf(stderr, "sc_compile: out of memory\n");
    ...
    ScLowerResult lr = sc_lower(combined, ccount, filename);
    ...
    if (lr.nerrors > 0) {
        fprintf(stderr, "sc_compile: %d lower error(s) in %s\n",
                lr.nerrors, filename);
        sc_lower_free(&lr);
        return NULL;
    }
