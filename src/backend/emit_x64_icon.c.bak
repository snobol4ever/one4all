...
static void emit_assign(EXPR_t *n, const char *γ, const char *ω,
    ...
    char ra[64],rb[64];
    emit_expr(n->children[1], rhs_ports_γ, rhs_ports_ω, ra, rb);
...
static void emit_suspend(EXPR_t *n, const char *γ, const char *ω,
    ...
    if (val_node) {
        ...
        emit_expr(val_node_γ, val_node_ω, vp, va, vb);
    } else {
    ...
    if (body_node) {
        ...
        emit_expr(body_node_γ, body_node_ω, bp, ba, bb);
        Ldef( resume_here);
        Jmp( ba);
    } else {
        Ldef( resume_here);
...
static void emit_call(EXPR_t *n, const char *γ, const char *ω,
                      char *oa, char *ob) {
    if(n->nchildren<1){ emit_fail_node(em, NULL_γ, NULL_ω, ports, oa,ob); return; }
    ...
    if(strcmp(fname,"write")==0){
        ...
        emit_expr(arg_γ, arg_ω, ap2, arg_a,arg_b);
        Ldef(a); Jmp(arg_a);
        Ldef(b); Jmp(arg_b);
        Ldef(after);
        if(arg->kind==E_QLIT){
    ...
    if(strcmp(fname,"match")==0 && !icn_is_user_proc(fname)){
        ...
        char aa[64],ab[64]; emit_expr(arg_γ, arg_ω, ap, aa,ab);
        Ldef(a); Jmp(aa);
        Ldef(b); Jmp(ω);
        Ldef(after);
    ...
    if(strcmp(fname,"find")==0 && nargs>=2 && !icn_is_user_proc(fname)){
        ...
        char a1[64],b1[64]; emit_expr(s1arg_γ, s1arg_ω, ap1, a1,b1);
        char ap2_γ[64]; char ap2_ω[64]; strncpy(ap2_γ,after2,63); strncpy(ap2_ω,ω,63);
        char a2[64],b2[64]; emit_expr(s2arg_γ, s2arg_ω, ap2, a2,b2);
...
static void emit_cset(EXPR_t *n, const char *γ, const char *ω,
                      char *oa, char *ob) {
    emit_str(em, n_γ, n_ω, ports, oa, ob);   /* same layout — char* in rdi, nothing pushed */
}
...
static void emit_scan(EXPR_t *n, const char *γ, const char *ω,
    ...
    char ea[64], eb[64];
    emit_expr(expr_node_γ, expr_node_ω, ep, ea, eb);
...
static void emit_while(EXPR_t *n, const char *γ, const char *ω,
    ...
    if(body){
        ...
        emit_expr(body_γ, body_ω, bp, ba,bb);
        loop_pop();
        Jmp(ba);
...
static void emit_every(EXPR_t *n, const char *γ, const char *ω,
    ...
    if(body){
        ...
        loop_push(ω, gbfwd);
        char ba[64],bb[64]; emit_expr(body_γ, body_ω, bp, ba,bb);
        loop_pop();
        char gp_γ[64]; char gp_ω[64]; strncpy(gp_γ,bstart,63); strncpy(gp_ω,ω,63);
        emit_expr(gen_γ, gen_ω, gp, ga,gb);
        Ldef(bstart); Jmp(ba);
    } else {
...
static void emit_augop(EXPR_t *n, const char *γ, const char *ω,
                       char *oa, char *ob) {
    if (n->nchildren < 2) { emit_fail_node(em, n_γ, n_ω, ports, oa, ob); return; }
    ...
    switch ((IcnTkKind)n->ival) {
        ...
        default:
            ...
            emit_fail_node(em, n_γ, n_ω, ports, oa, ob); return;
    }
    ...
    char op_ports_γ[64]; char op_ports_ω[64];
    strncpy(op_γ, store, 63);
    strncpy(op_ω, ω, 63);

    char opa[64], opb[64];
    emit_expr(&syn, op_ports_γ, op_ports_ω, opa, opb);
...
static void emit_nonnull(EXPR_t *n, const char *γ, const char *ω,
    ...
    emit_expr(child_γ, child_ω, cp, ca, cb);
    Ldef(a); Jmp(ca);
    Ldef(b); Jmp(cb);
}
...
static void emit_repeat(EXPR_t *n, const char *γ, const char *ω,
    ...
    if (body) emit_expr(body_γ, body_ω, bp, ba, bb);
    loop_pop();
    Ldef(a);
    Ldef(top);
...
static void emit_limit(EXPR_t *n, const char *γ, const char *ω,
    ...
    emit_expr(lim_γ, lim_ω, lp, la,lb);
...
static void emit_case(EXPR_t *n, const char *γ, const char *ω,
    ...
    emit_expr(sel_γ, sel_ω, sp, sa,sb);
    Ldef(a); Jmp(sa);
    Ldef(b); Jmp(sb);
    Ldef(got_sel);
    E("    pop     rax\n");
    ...
    for (int i = 0; i < npairs; i++) {
        ...
        emit_expr(key_γ, key_ω, kp, ka,kb);
        Jmp(ka);
        Ldef(got_key);
        E("    pop     rax\n");
        E("    cmp     rax, [rbp%+d]\n", slot_offset(sel_slot));
        ...
        emit_expr(body_γ, body_ω, bp, ba2,bb2);
        Jmp(ba2);
        Ldef(next_arm);
    }
    if (has_default) {
        ...
        emit_expr(def_body_γ, def_body_ω, dp, da,db);
        Jmp(da);
    } else {
        Jmp(ω);
    }
...
static void emit_bang(EXPR_t *n, const char *γ, const char *ω,
    ...
    if (child) emit_expr(child_γ, child_ω, cp, ca, cb);
...
static void emit_cset_binop(EXPR_t *n, const char *γ, const char *ω,
    ...
    char ra[64], rb[64];
    char rp_γ[64]; char rp_ω[64]; strncpy(rp_γ, compute, 63); strncpy(rp_ω, lbfwd, 63);
    emit_expr(rch_γ, rch_ω, rp, ra, rb);
    char la[64], lb[64];
    char lp_γ[64]; char lp_ω[64]; strncpy(lp_γ, lstore, 63); strncpy(lp_ω, ω, 63);
    emit_expr(lch_γ, lch_ω, lp, la, lb);
    Ldef( lbfwd); Jmp( lb);
    Ldef( a); Jmp( la);
    Ldef( b); Jmp( rb);
    Ldef( lstore);
...
static void emit_expr(EXPR_t *n, const char *γ, const char *ω,
                      char *oa, char *ob) {
    if(!n){ emit_fail_node(em, n_γ, n_ω, ports, oa,ob); return; }
    switch(n->kind){
        case E_ILIT:    emit_int      (em,n,ports,oa,ob); break;
        case E_QLIT:    emit_str      (em,n,ports,oa,ob); break;
        case E_CSET:   emit_cset     (em,n,ports,oa,ob); break;
        case E_VAR:    emit_var      (em,n,ports,oa,ob); break;
        case E_ASSIGN: emit_assign   (em,n,ports,oa,ob); break;
        case E_RETURN: emit_return   (em,n,ports,oa,ob); break;
        case E_SUSPEND:emit_suspend  (em,n,ports,oa,ob); break;
        case E_FAIL:   emit_fail_node(em, n_γ, n_ω, ports, oa,ob); break;
        case E_IF:     emit_if       (em,n,ports,oa,ob); break;
        case E_GENALT:    emit_alt      (em,n,ports,oa,ob); break;
        case E_MATCH:   emit_scan     (em,n,ports,oa,ob); break;
        case E_NEG:    emit_neg      (em,n,ports,oa,ob); break;
        case E_NOT:    emit_not      (em,n,ports,oa,ob); break;
        case E_NULL:   emit_not      (em,n,ports,oa,ob); break;
        case E_SSEQ:    emit_seq      (em,n,ports,oa,ob); break;
        ...
        case E_TO:     emit_to       (em,n,ports,oa,ob); break;
        case E_TO_BY:  emit_to_by    (em,n,ports,oa,ob); break;
        case E_EVERY:  emit_every    (em,n,ports,oa,ob); break;
        case E_WHILE:  emit_while    (em,n,ports,oa,ob); break;
        case E_UNTIL:  emit_until    (em,n,ports,oa,ob); break;
        case E_FNC:   emit_call     (em,n,ports,oa,ob); break;
        case E_AUGOP:  emit_augop    (em,n,ports,oa,ob); break;
        /* G-9 gap-fill cases */
        case E_NONNULL:   emit_nonnull   (em,n,ports,oa,ob); break;
        case E_FLIT:      emit_real      (em,n,ports,oa,ob); break;
        case E_SIZE:      emit_size      (em,n,ports,oa,ob); break;
        case E_POW:       emit_pow       (em,n,ports,oa,ob); break;
        case E_SEQ_EXPR:  emit_seq_expr  (em,n,ports,oa,ob); break;
        case E_IDENTICAL: emit_identical (em,n,ports,oa,ob); break;
        case E_SWAP:      emit_swap      (em,n,ports,oa,ob); break;
        case E_SGT: case E_SGE: case E_SLT:
        case E_SLE: case E_SNE:
                            emit_strrelop  (em,n,ports,oa,ob); break;
        case E_REPEAT:    emit_repeat    (em,n,ports,oa,ob); break;
        case E_LOOP_BREAK:     emit_break_node(em, n_γ, n_ω, ports, oa,ob); break;
        case E_LOOP_NEXT:      emit_next_node (em,n,ports,oa,ob); break;
        case E_INITIAL:   emit_initial   (em,n,ports,oa,ob); break;
        case E_LIMIT:     emit_limit     (em,n,ports,oa,ob); break;
        case E_IDX: emit_subscript (em,n,ports,oa,ob); break;
        case E_SECTION: case E_SECTION_PLUS: case E_SECTION_MINUS:
                            emit_section   (em,n,ports,oa,ob); break;
        case E_MAKELIST:  emit_makelist  (em,n,ports,oa,ob); break;
        case E_RECORD:    emit_record    (em,n,ports,oa,ob); break;
        case E_FIELD:     emit_field     (em,n,ports,oa,ob); break;
        case E_CASE:      emit_case      (em,n,ports,oa,ob); break;
        case E_ITER:      emit_bang      (em,n,ports,oa,ob); break;
        case E_BANG_BINARY: emit_stub_fail(em, n_γ, n_ω, ports, oa,ob); break;
        /* G1: ICN_POS — unary plus, identity: emit child unchanged */
        case E_PLS:    emit_expr(n->children[0], ports_γ, ports_ω, oa, ob); break;
        /* G2: ICN_RANDOM — ?E: random integer 1..E via icn_random() */
        case E_RANDOM: emit_random(em, n_γ, n_ω, ports, oa,ob); break;
        ...
        case E_CSET_COMPL:  emit_cset_complement(em, n_γ, n_ω, ports, oa,ob); break;
        case E_CSET_UNION:  emit_cset_binop(em, n_γ, n_ω, ports, oa,ob); break;
        case E_CSET_DIFF:   emit_cset_binop(em, n_γ, n_ω, ports, oa,ob); break;
        case E_CSET_INTER:  emit_cset_binop(em, n_γ, n_ω, ports, oa,ob); break;
        case E_MATCH:     emit_match     (em,n,ports,oa,ob); break;
        case E_SEQ: {
 ...
 * ======================================================================= */
void icn_emit_file(EXPR_t **nodes, int count) {
    ...
    /* Emit to temp buffer */
    FILE *tmp=tmpfile(); FILE *real=f; out=tmp; uid=0;
    ...
    for(int pi=0;pi<count;pi++){
        ...
        for(int i=nstmts-1;i>=0;i--){
            ...
            char sa[64],sb[64]; emit_expr(stmt_γ, stmt_ω, sp, sa,sb);
