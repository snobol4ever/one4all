...
# under --ir-run and --interp must also pass under --run.  Target state
...
read JIT_PASS JIT_FAIL <<< "$(run_mode --run "$TMPDIR/jit")"
...
# Gate: --interp and --run must match --ir-run PASS count.
...
if [ "$SM_PASS" -lt "$IR_PASS" ]; then
    echo "FAIL  --interp PASS ($SM_PASS) < --ir-run PASS ($IR_PASS)"
    GATE_FAIL=1
fi
if [ "$JIT_PASS" -lt "$IR_PASS" ]; then
    echo "FAIL  --run PASS ($JIT_PASS) < --ir-run PASS ($IR_PASS)"
    GATE_FAIL=1
fi
