...
def load_ignore_rules(path):
    """Read IGNORE rules from a tracepoints.conf-format file.

    Rule line:    IGNORE  <name-regex>  <value-regex>

    When an event's (name, value) pair matches any rule, the controller
    canonicalises the value to '<IGNORED>' before comparing across
    participants — masking known cross-dialect representation differences
    without losing the event's existence/order.
    """
    rules = []
    ...
    with open(path) as f:
        for raw in f:
            ...
            if len(parts) >= 3 and parts[0].upper() == 'IGNORE':
                try:
                    rules.append((re.compile(parts[1], re.IGNORECASE),
                                  re.compile(parts[2], re.IGNORECASE)))
                except re.error:
                    pass
    return rules

def value_after_ignore(name, value, rules):
    for name_rx, val_rx in rules:
        if name_rx.fullmatch(name) and val_rx.fullmatch(value):
            return '<IGNORED>'
    return value
...
def main():
    ...
    # Load IGNORE rules (optional) — canonicalise known cross-dialect noise
    ignore_rules = load_ignore_rules(os.environ.get('MONITOR_IGNORE_FILE'))
    if ignore_rules:
        print(f"[sync monitor] loaded {len(ignore_rules)} IGNORE rule(s)", flush=True)

    # Open per-participant event log (best-effort; controller env may set this)
    log_dir = os.environ.get('MONITOR_LOG_DIR')
    log_fps = []
    if log_dir and os.path.isdir(log_dir):
        for nm in names:
            log_fps.append(open(os.path.join(log_dir, f'{nm}.events.log'), 'w'))
    else:
        log_fps = [None] * n
        ...
        while remaining:
            ...
            for fobj in readable:
                ...
                if rec is None:
                    ...
                    name_u = name.upper()
                    canon_value = value_after_ignore(name_u, value, ignore_rules)
                    events[i] = f'{kind}\x1f{name_u}\x1f{canon_value}'
                    if log_fps[i] is not None:
                        tag = ' [IGN]' if canon_value != value else ''
                        log_fps[i].write(f'{step+1:6d}  {kind:8s} {name_u:24s} = {value!r}{tag}\n')
                        log_fps[i].flush()
                    remaining.remove(i)
