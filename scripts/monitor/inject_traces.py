...
def build_trace_registrations(functions, variables):
    ...
    # SPITBOL x64 quirk: TRACE rejects a quoted string as 2nd arg (Error 199).
    # Bare keyword reference (CALL / RETURN / VALUE — resolves to integer code)
    # is accepted by both SPITBOL and CSNOBOL4.  See test_monitor_2way_sync_step
    # session note 2026-04-26.
    for fn in functions:
        lines.append(f"        TRACE('{fn}',CALL,   '','MONCALL')\n")
        lines.append(f"        TRACE('{fn}',RETURN, '','MONRET')\n")
    # 4-arg TRACE(var,VALUE,'',MONVAL) — fires MONVAL callback on every assign.
    # 2-arg form was historically emitted for scrip's trace_set; scrip does not
    # execute the preamble's LOAD()/TRACE() at all today, so we drop the 2-arg
    # form (it is redundant for the oracles and unused for scrip).
