...
# include <fcntl.h>			/* for O_WRONLY etc (TRACE_STREAM) */
...
#ifdef TRACE_STREAM
    {
        extern int g_csno_trace, g_csno_trace_fd;
        if (getenv("SNO_TRACE")) {
            g_csno_trace_fd = open("/tmp/sno_csno.trace",
                                   O_WRONLY|O_CREAT|O_TRUNC, 0644);
            if (g_csno_trace_fd < 0) g_csno_trace_fd = 2;
            g_csno_trace = 1;
        }
    }
#endif
    ret = snobol4_init(argc, argv, 1);
    if (ret != INIT_OK)
	return ret;
