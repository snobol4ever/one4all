...
#
# Compiles each .pro in a given directory via sno2c -pl -jvm, assembles
...
if [[ ! -x "$SNO2C" ]]; then echo "ERROR: sno2c not found at $SNO2C"; exit 1; fi
...
run_test() {
    ...
    # Compile .pro -> .j
    if ! "$SNO2C" -pl -jvm "$pro" > "$jfile" 2>"$WORK/${base}.sno2c_err"; then
