/*
 * bench_pcre2_wins.c — Where does PCRE2 beat scrip-cc?
 ...
 * scrip-cc naive scan has no such optimization yet.
 * ------------------------------------------------------------------- */
 ...
 * scrip-cc would need full ARBNO+ALT chains — more overhead.
 * ------------------------------------------------------------------- */
 ...
 * scrip-cc: character-by-character
 * ------------------------------------------------------------------- */
 ...
 * scrip-cc: function call overhead + loop
 * ------------------------------------------------------------------- */
...
int main(void) {
    printf("=================================================================\n");
    printf("  WHERE DOES PCRE2 BEAT scrip-cc? — Honest Benchmarks\n");
    printf("=================================================================\n\n");
    ...
    printf("  scrip-cc (naive scan) : %7.2f ns\n", tA_tiny);
    printf("  PCRE2 JIT (Boyer-Moore)   : %7.2f ns\n", tA_pcre);
    if(tA_pcre<tA_tiny)
        printf("  Result: PCRE2 wins by %.2fx\n\n", tA_tiny/tA_pcre);
    else
        printf("  Result: scrip-cc wins by %.2fx\n\n", tA_pcre/tA_tiny);
    ...
    printf("  scrip-cc (hand-written): %7.2f ns\n", tB_tiny);
    ...
    printf("  scrip-cc (direct cmp)  : %7.2f ns\n", tD_tiny);
    printf("  PCRE2 JIT                  : %7.2f ns\n", tD_pcre);
    if(tD_pcre<tD_tiny)
        printf("  Result: PCRE2 wins by %.2fx\n\n", tD_tiny/tD_pcre);
    else
    ...
    printf("     when scrip-cc has no string search optimization built in.\n");
    printf("  B. Complex character classes — PCRE2 JIT compiles [a-z0-9._%+-]\n");
    printf("     to a 256-entry bitmask lookup. scrip-cc uses branching.\n");
    printf("  C. SIMD no-match scan    — PCRE2 JIT uses SSE/AVX to scan 16-32\n");
    printf("     bytes at once. scrip-cc scans one char at a time.\n");
    ...
    printf("  scrip-cc has none of these YET. They are all addable.\n");
